...
| 组件 | 职责 |
|------|------|
| **mason.nvim** | 安装、升级、查看 LSP 与 CLI（jdtls、typescript-language-server、pyright/basedpyright、ruff、prettier、google-java-format 等） |
...
| **conform.nvim** | 按文件类型调用格式化器；**Java 固定为 google-java-format**；TS/JS 可用 **prettier**；Python 可用 **ruff format** 或保留 LSP（实现时定默认，建议 ruff format 与 ruff lint 一致） |
| **nvim-cmp** | 插入模式补全 |
...
- **Formatter / 工具**：`google-java-format`、`prettier`、`ruff`（ruff 可同时承担 format 与 lint 时由 conform 映射）
...
- 默认 **prettier**（Mason）；若某仓库无 Prettier 配置，实现时可 fallback 到 **LSP format** 或仅提示（在实现计划中择一，避免双标准 silently 分叉）。
...
- **TypeScript**：Monorepo 尽量与团队打开目录一致，保证 `tsconfig` / 路径别名解析正确。
...
1. **占位符**：无 TBD；语言默认在 §2、§5 已写明 Java 用 google-java-format。  
2. **一致性**：Java 格式化以 conform + google-java-format 为准，与「不用 jdtls 作为唯一 format 来源」一致。  
3. **范围**：单 spec 覆盖 TS/Java/Python + Mason + 补全 + 格式化；调试与 DAP 明确排除。  
4. **歧义**：TS/JS、Python 在「无 prettier / 无 ruff」时的 fallback 在实施计划中必须写死为一种行为。
