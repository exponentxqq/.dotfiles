...
void test("parseFrontmatter handles quoted description with colon", () => {
  const content = `---
name: pptx
...
void test("parseFrontmatter returns null for missing frontmatter", () => {
  assert.equal(parseFrontmatter("# no frontmatter"), null);
});

void test("parseFrontmatter returns null without name or description", () => {
  assert.equal(parseFrontmatter("---\nname: x\n---\n"), null);
});

void test("parseFrontmatter reads multi-line folded description", () => {
  const md = "---\nname: demo\ndescription: >-\n  line one\n  line two\n---\nbody";
